...
class SharedRMSprop(optim.Optimizer):
    ...
    def __init__(self,
                 params,
                 lr=7e-4,
                 alpha=0.99, # tương ứng decay factor (thường gọi là rho trong RMSprop)
                 eps=0.1, # là 1 số nhỏ để tránh chia cho 0 (ở đây là 0.1 khá lớn)
                 weight_decay=0,
                 momentum=0,
                 centered=False): # nếu True, dùng phiên bản centered RMSprop (bình phương trừ bình phương trung bình)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                exp_avg_hat = exp_avg / (1 - beta1**state['step'].item())
                ...
                if amsgrad:
                    ...
                    denom = max_exp_avg_sq / (1 - beta2**state['step'].item())
                else:
                    denom = exp_avg_sq / (1 - beta2**state['step'].item())

                p.data.addcdiv_(-group['lr'], exp_avg_hat, denom.sqrt().add_(group['eps']))
